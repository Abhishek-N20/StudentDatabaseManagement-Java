...
public class StudentDAO {
	...
	public static int save(Student s){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into student_database(name,USN,sem,branch,sec,email,mob) values(?,?,?,?,?,?,?)");
	...
	public static int update(Student s){
		...
		try{
			Connection con=AdminDAO.getCon();
			PreparedStatement ps=con.prepareStatement("update student_database set name=?,sem=?,branch=?,sec=?,email=?,mob=? where USN=?");
			ps.setString(1,s.getName());
			ps.setInt(2, s.getSem());
			ps.setString(3, s.getBranch());
			ps.setString(4,s.getSection());
			ps.setString(5,s.getEmail());
			ps.setInt(6, s.getMobile());
	...
	public static Student getStudentByUSN(String USN){
		...
		try{
			Connection con=AdminDAO.getCon();
			PreparedStatement ps=con.prepareStatement("select * from student_database where USN=?");
			ps.setString(1,USN);
